...
# Fonts
Tharlon = "assets/fonts/Tharlon-Regular.ttf"
smallFont = pygame.font.Font(Tharlon, 20)
largeFont = pygame.font.Font(Tharlon, 40)

letters = {}
with open("mmfilenames.csv", mode="r", encoding="utf-8") as f:
    reader = csv.reader(f)
    next(reader)
    for row in reader:
        letters[int(row[0])] = str(row[1])
...
while True:
    ...
    if classification is not None:
        classificationText = largeFont.render(str(letters[int(classification)]), True, WHITE)
